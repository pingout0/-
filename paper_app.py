...
# 다크/라이트 모드 양쪽에서 잘 보이도록 그래프 공통 테마
...
if section.startswith("3.1"):
    st.markdown('<div class="hero"><b>논문 3.1절 · 데이터 탐색적 분석</b><br>'
                '그림 1~3 계산 확인</div>', unsafe_allow_html=True)
    ...
    with tab1:
        ...
        fig.update_layout(**THEME, height=380, margin=dict(t=20,b=20), yaxis_title="LME ($/톤)")
        st.plotly_chart(fig, use_container_width=True)
        # 국면별 통계 실시간 계산
        ret = df['LME가격'].pct_change()*100
        st.markdown("**국면별 특성 (계산)**")
    ...
    with tab2:
        ...
        st.markdown('<div class="step">각 변수를 시간축에서 밀어보며 LME와 가장 닮는 지점을 찾습니다. </div>', unsafe_allow_html=True)
        if st.button("🔄 상관 실시간 계산", key="corr"):
            with st.spinner("계산 중..."):
                ...
                fig.update_layout(**THEME, height=340,margin=dict(t=20,b=20),xaxis_title="시차(개월): 음수=선행",yaxis_title="상관")
                st.plotly_chart(fig,use_container_width=True)
                # 수준 vs 변화율
    ...
    with tab3:
        ...
        st.markdown('<div class="step">월별 평균 변화율의 차이가 통계적으로 유의한지 ANOVA로 검정.</div>', unsafe_allow_html=True)
        if st.button("🔄 계절성 ANOVA 검정 실행", key="anova"):
            with st.spinner("검정 중..."):
                ...
                fig.update_layout(**THEME, height=320,margin=dict(t=20,b=20),xaxis_title="월",yaxis_title="평균 변화율(%)")
                st.plotly_chart(fig,use_container_width=True)
                c1,c2=st.columns(2)
...
# ══════════════════════════════════════════════
    ...
    st.markdown('<div class="hero"><b>논문 3.2절 · 네 가지 예측 방법</b><br>'
                '각 방법이 어떻게 작동하는지 실행.</div>', unsafe_allow_html=True)
    ...
    if st.button("▶ 이 방법 실행", type="primary"):
        ...
        with st.spinner("모델 학습·예측 중..."):
            ...
            fig.update_layout(**THEME, height=360,margin=dict(t=20,b=20),yaxis_title="LME ($/톤)",legend=dict(orientation="h",y=1.1))
            st.plotly_chart(fig,use_container_width=True)
            m=evaluate(yte,pred)
...
# ══════════════════════════════════════════════
    ...
    st.markdown('<div class="hero"><b>논문 3.3절 · 평가지표 도출</b><br>'
                '각 지표가 어떻게 계산되는지 수식과 함께 확인</div>', unsafe_allow_html=True)
    st.markdown("""
...
# ══════════════════════════════════════════════
    ...
    st.markdown('<div class="hero"><b>논문 4.1절 · 방법 성능 비교</b><br>'
                '표 2 재현 (전 방법 × 전 기간)</div>', unsafe_allow_html=True)
    st.markdown('<div class="step">아래 버튼을 누르면 4가지 방법을 3가지 기간(단기·중기·장기)에 대해 모두 계산. </div>',unsafe_allow_html=True)
    if st.button("▶ 논문 표 2 전체 재현", type="primary"):
        ...
        st.markdown('<div class="res">💡 계산된 결과 RSM 단독이 단기·중기에서 최우수 </div>',unsafe_allow_html=True)
...
# ══════════════════════════════════════════════
    ...
    st.markdown('<div class="hero"><b>논문 4.2절 · 계수 예측의 한계</b><br>'
                'RSM 계수가 왜 예측 불가능한지 직접 확인 (논문 그림 5)</div>', unsafe_allow_html=True)
    ...
    if st.button("▶ 계수 변동성 실시간 측정", type="primary"):
        ...
        with st.spinner("이동 윈도우로 계수 반복 추정 중..."):
            ...
            fig.update_layout(**THEME, height=340,margin=dict(t=20,b=20),yaxis_title="변동계수 CV (log)",yaxis_type="log")
            st.plotly_chart(fig,use_container_width=True)
...
# ══════════════════════════════════════════════
    ...
    st.markdown('<div class="hero"><b>논문 4.4절 · 구간 분리 분석</b><br>'
                '국면별로 최적 방법이 다름을 확인 (표 3)</div>', unsafe_allow_html=True)
    ...
    if st.button("▶ 구간별 방법 실시간 비교", type="primary"):
        ...
        st.markdown('<div class="res">💡 구간마다 최적 방법이 다름 — 안정기는 RSM, 급등기는 2단계, 조정기는 직접예측. "항상 맞는 방법은 없음"을 확인</div>',unsafe_allow_html=True)
...
# ══════════════════════════════════════════════
    ...
    import json
    st.markdown('<div class="hero"><b>통합 에이전트 · 데이터 넣으면 최적 조합 자동 선택</b><br>'
                '데이터 구성 × 예측 방법을 모두 탐색 → 강건하게 최적 선택</div>', unsafe_allow_html=True)

    try:
        A = json.load(open("agent_precomputed.json"))
    except Exception:
        st.warning("agent_precomputed.json이 필요합니다.")
        st.stop()

    st.markdown('<div class="step">에이전트는 자르기(유사구간 합치기)·증강·예측방법을 모두 조합해보고, 여러 검증 시점에서 일관되게 좋은 조합을 자동으로 고릅니다.</div>', unsafe_allow_html=True)
    ...
    future = d["future"]
    n_hist = len(hist)
    fig.add_trace(go.Scatter(x=list(range(n_hist)), y=hist, name="실제(최근24개월)",
                             line=dict(color=NAVY, width=2)))
    fig.add_trace(go.Scatter(x=list(range(n_hist-1, n_hist+len(future))),
                             y=[hist[-1]]+future, name="에이전트 예측",
                             line=dict(color=COPPER, width=2, dash="dash")))
    fig.update_layout(**THEME, height=340, margin=dict(t=20, b=20),
                      yaxis_title="가격", legend=dict(orientation="h", y=1.1))
    st.plotly_chart(fig, use_container_width=True)

    # 상위 조합 비교
    st.markdown("### 탐색한 조합들 (상위 8개, 강건 점수 순)")
    rows = [{"데이터 구성": c["data"], "방법": c["method"],
             "MAPE(%)": c["mape"], "시점편차": c["std"]} for c in d["combos"]]
    st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)

    st.markdown('<div class="step">💡 개별 요소로는 안 보이던 최적 조합이 결합 탐색에서 드러납니다. '
                '비트코인은 자르기·방법을 각각 볼 땐 전체사용이 최선(46%)이었으나, '
                '유사구간+증강+XGBoost 조합에서 12%로 급감했습니다.</div>', unsafe_allow_html=True)
